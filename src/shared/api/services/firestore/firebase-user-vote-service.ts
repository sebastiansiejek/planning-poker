...
import type { UserVoteService } from '@/shared/factories/user-vote-service-factory';
import type { Vote } from '@/shared/types/types';

export class FirebaseUserVoteService implements UserVoteService {
  getVotedUsers: UserVoteService['getVotedUsers'] = async (gameId, roomId): Promise<{ userId: string;  vote: string; }[]> =>
  {
    if (!roomId) {
